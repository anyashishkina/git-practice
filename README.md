...
![](/images/1.png)
4)	Во вкладке оборудование ставим размер ОЗУ 1 ГБ, Процессор по умолчанию, Во вкладке жесткий диск указываем размер диск 15 ГБ
![](/images/2.png)
![](/images/3.png)
6)	Переходим в вкладку Расширенные – Сеть, проверяем, чтобы стояла галочка включить сетевой адаптер 1, все остальные адаптеры отключаем и в поле тип подключения выбираем Виртуальный адаптер, все остальное оставляем как есть
![](/images/5.png)
7)	Нажимаем кнопку Свойства Адаптера и выбираем вкладку DHCP сервер, и снимаем галочку Включить сервер
![](/images/7.png)
8)	Настраиваем Ubuntu
9)	Создаём учётную запись
10)	Настраиваем сеть: Выключим виртуальную машину, и зайдем ее сеть Файл – Менеджер Сетей - Свойства – Сети NAT. Во вкладке основные опции введем имя сети NatNetwork, далее введем IP нашей сети с маской подсети 24 – 172.16.1.0/24, обязательно снимем галочку включить DCHP. Теперь перейдем в настройки сети самой виртуальной машины и выберем тип подключения сеть NAT остальное оставим по умолчанию
11)	Запустим виртуальную машину , откроем терминал , введем команду IP A и убедимся что у нас есть сетевой адаптер и у него есть физический адрес(MAC)
![](/images/7.png)
